...
$(function() {
		var r = $("div.msg");
		var m = r.html() + ": nouveau message dynamique.";
		r.html(m).css('color','green');
...
//rep objet $ représentant le paragraphe de la div panier
	if (rep == repGood) {
		...
		//out.html("Bonne reponse");
		alert('Bonne reponse!');
		question();
		clearTimeout(timer);
	} else {
		out.html("Mauvaise reponse");
	}
...
function prepare_div_event() { //fonction lancée après le chargt des objets-balises en mémoire
	...
	$(function() {
		...
		drop: function( event, ui ) {
			...
			var d = ui.draggable; // ui.draggable() donne un objet $ sur la div déplacée
			var i = d.attr('id');  // attr donne la valeur de son attribut id
			
			var repGood =  tabObject[numQuestion].reponse;
			
			var out = drp.find("p");
			reponse(i, repGood, out);
		}
		});
	});
}
